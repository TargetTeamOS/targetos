...
// Try every possible env var name for Supabase
...
module.exports = async function handler(req, res) {
  ...
  // Default greeting - works even without DB
  const defaultGreeting = 'Thank you for calling Target Team. For sales press 1. For any available agent press 0. To leave a voicemail press 9.'

  const supabase = getSupabase()

  // If no DB connection, still answer the call with default IVR
  if (!supabase) {
    console.error('Supabase not configured - SUPABASE_URL and SUPABASE_SERVICE_KEY must be set in Vercel')
    return res.send(wrap(
      `<Gather numDigits="1" action="/api/twilio-menu" method="POST" timeout="10">` +
        say(defaultGreeting) +
      `</Gather>` +
      say('We did not receive your selection. Please call back.')
    ))
  }
  ...
  try {
    ...
    const { data: contact } = await supabase
      .from('contacts').select('id, first_name, last_name, agent_id, agents(id,name)')
    ...
    if (contact && contact.agent_id) {
      const { data: ext } = await supabase.from('phone_extensions').select('*')
    ...
    // Play IVR menu
    const greeting = (ivr && ivr.greeting_text) || defaultGreeting
    return res.send(wrap(
      '<Gather numDigits="1" action="/api/twilio-menu" method="POST" timeout="10">' +
        say(greeting) +
      '</Gather>' +
    ...
    console.error('twilio-inbound DB error:', err.message)
    // Still answer the call even if DB fails
    return res.send(wrap(
      '<Gather numDigits="1" action="/api/twilio-menu" method="POST" timeout="10">' +
        say(defaultGreeting) +
      '</Gather>'
