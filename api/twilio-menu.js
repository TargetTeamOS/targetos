// /api/twilio-menu — IVR keypress router
import { createClient } from '@supabase/supabase-js'
const supabase = createClient(
  process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL,
  process.env.SUPABASE_SERVICE_KEY || process.env.VITE_SUPABASE_ANON_KEY
)
const twiml = (xml) => `<?xml version="1.0" encoding="UTF-8"?><Response>${xml}</Response>`
const say   = (t)   => `<Say voice="Polly.Joanna">${t}</Say>`
const vmRecord = `${say('Please leave your message after the tone.')}<Record maxLength="120" transcribe="true" transcribeCallback="/api/twilio-voicemail" />`

export default async function handler(req, res) {
  res.setHeader('Content-Type', 'text/xml')
  if (req.method !== 'POST') return res.status(405).end()

  const digits = req.body?.Digits || ''
  const to     = req.body?.To     || ''

  try {
    const { data: ivr } = await supabase.from('phone_ivr').select('*').eq('is_active', true).maybeSingle()
    const options = ivr?.menu_options || []
    const chosen  = options.find(o => String(o.key) === String(digits))

    if (!chosen) {
      return res.status(200).send(twiml(
        `${say('Invalid selection.')}
         <Gather numDigits="1" action="/api/twilio-menu" method="POST" timeout="8">
           ${say(ivr?.greeting_text || 'Please try again.')}
         </Gather>`
      ))
    }

    if (chosen.action === 'voicemail' || digits === (ivr?.voicemail_extension || '9')) {
      return res.status(200).send(twiml(vmRecord))
    }

    if (chosen.action === 'extension') {
      const { data: ext } = await supabase
        .from('phone_extensions')
        .select('*')
        .eq('number', String(chosen.value))
        .eq('active', true)
        .maybeSingle()

      if (ext?.forward_to) {
        return res.status(200).send(twiml(
          `${say('Connecting you to ' + ext.label + '.')}
           <Dial callerId="${to}" timeout="30" record="record-from-answer" recordingStatusCallback="/api/twilio-status">
             <Number>${ext.forward_to}</Number>
           </Dial>
           ${ext.voicemail_greeting ? say(ext.voicemail_greeting) : say('Please leave a message.')}
           <Record maxLength="120" transcribe="true" transcribeCallback="/api/twilio-voicemail" />`
        ))
      }
      return res.status(200).send(twiml(`${say('That extension is unavailable.')} <Redirect>/api/twilio-inbound</Redirect>`))
    }

    if (chosen.action === 'round_robin') {
      const { data: rule } = await supabase.from('phone_routing').select('*').eq('rule_type', 'round_robin').eq('is_active', true).maybeSingle()
      const agentIds = rule?.config?.agent_ids || []
      if (agentIds.length > 0) {
        const { data: recentCalls } = await supabase.from('calls').select('agent_id').eq('direction', 'Inbound').in('agent_id', agentIds).order('called_at', { ascending: false }).limit(100)
        const counts = {}
        agentIds.forEach(id => { counts[id] = 0 })
        ;(recentCalls || []).forEach(c => { if (counts[c.agent_id] !== undefined) counts[c.agent_id]++ })
        const nextId = agentIds.reduce((a, b) => counts[a] <= counts[b] ? a : b)
        const { data: ext } = await supabase.from('phone_extensions').select('*').eq('agent_id', nextId).eq('active', true).maybeSingle()
        if (ext?.forward_to) {
          return res.status(200).send(twiml(
            `${say('Connecting you to the next available agent.')}
             <Dial callerId="${to}" timeout="30" record="record-from-answer" recordingStatusCallback="/api/twilio-status">
               <Number>${ext.forward_to}</Number>
             </Dial>
             ${say('Please leave a message.')}
             <Record maxLength="120" transcribe="true" transcribeCallback="/api/twilio-voicemail" />`
          ))
        }
      }
      return res.status(200).send(twiml(`${say('All agents are unavailable. Please leave a message.')} ${vmRecord}`))
    }

    // repeat or fallback
    return res.status(200).send(twiml(`<Redirect>/api/twilio-inbound</Redirect>`))

  } catch(err) {
    console.error('twilio-menu error:', err)
    return res.status(200).send(twiml(say('An error occurred. Please call back.')))
  }
}
