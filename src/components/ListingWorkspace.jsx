...
export default function ListingWorkspace({
  ...
  return (
    ...
    <div style={{ fontFamily:ff, position:'relative', left:'50%', right:'50%', marginLeft:'-50vw', marginRight:'-50vw', width:'100vw', minHeight:'100vh', marginTop:-28, background:'var(--bg)' }}>
      ...
      <div style={{ background:'linear-gradient(180deg, var(--panel), var(--bg))', borderBottom:'1px solid var(--border)', borderTop:'4px solid '+sc }}>
        <div style={{ maxWidth:1280, margin:'0 auto', padding:'18px 32px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:12, marginTop:18 }}>
            {statTile('Price', listing.list_price?fmt$(listing.list_price):'—', listing.original_price&&listing.original_price!==listing.list_price?'orig '+fmt$(listing.original_price):null, null, 'price')}
            {statTile('Days on market', d!=null?d:'—', d!=null?'days':null, null, 'overview')}
            ...
            {statTile('Last seller update', listing.seller_updated_at?fmtDate(listing.seller_updated_at):'never', null, listing.seller_updated_at?'var(--text)':'#DC2626', 'report')}
          </div>
        </div>
      </div>
      ...
      {tab==='overview' && (
        <div>
          {/* Needs attention / next action strip */}
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px', borderRadius:12, marginBottom:16,
            ...
            <span style={{ fontSize:20 }}>{sellerOverdue ? '⚠️' : '✅'}</span>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:10.5, fontWeight:800, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'.04em' }}>Next action</div>
              <div style={{ fontSize:14, fontWeight:700, color:'var(--text)' }}>{recommendation}</div>
            </div>
      ...
      {/* ── BUYER FEEDBACK (showings grouped by agent + open houses) ── */}
      {tab==='feedback' && (
        <div>
