...
function RightPanel({ contact: f, contactId, navigate, relDeals, relListings = [], relOffers = [], relTasks, agents, agent, voiceNotes = [], onRefreshTimeline, layout, editLayout, setLayout }) {
  ...
  return (
    ...
    <RightSection hideKey="appointments" layout={layout} editLayout={editLayout} onReorder={onReorder} onHide={onHide} title="Appointments" icon="📅" color="#8B5CF6"
      ...
      {/* ── RELATED LISTINGS (seller) ── */}
      {relListings.length > 0 && (
        <RightSection hideKey="listings" layout={layout} editLayout={editLayout} onReorder={onReorder} onHide={onHide} title={"Listings (" + relListings.length + ")"} icon="🏡" color="#F5A623">
          {relListings.map(l => (
            <div key={l.id} onClick={() => navigate('/listings')}
              style={{ padding:'7px 0', borderBottom:'1px solid var(--border)', cursor:'pointer' }}>
              <div style={{ fontSize:'12px', fontWeight:600, color:'var(--text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{l.addr}{l._primary && <span style={{ marginLeft:5, fontSize:9, fontWeight:700, color:'#0B7A45' }}>PRIMARY</span>}</div>
              <div style={{ display:'flex', justifyContent:'space-between', marginTop:'3px' }}>
                <span style={{ fontSize:'11px', fontWeight:700, color:'#F5A623' }}>{l.list_price ? fmt$(l.list_price) : '—'}</span>
                <Pill label={l._role || l.status || 'seller'} color="#9aadbd" />
              </div>
            </div>
          ))}
        </RightSection>
...
export function ContactDetail() {
  ...
  async function loadRelated() {
    try {
      ...
      setRelDeals(deals)
      setRelTasks(tasks)
      // Related listings (seller contact links)
      try {
  ...
  async function saveEngagementField(field, value, opts = {}) {
    ...
    try {
      ...
      // FIX (post-launch review, Sept 2026): this used to also match
      // any error message containing the substring "contact_engagements"
      // -- which a unique-constraint violation (e.g. the race in
      // db.engagements.startWorking, now fixed) or a permission error
      // on this table would ALSO contain, silently misclassifying a
      // real failure as "table doesn't exist yet" and writing the
      // field onto the shared `contacts` row instead -- exactly the
      // private-data leak this feature exists to prevent. Narrowed to
      // the precise "relation does not exist" signal only, matching
      // the pattern already used correctly in Contacts.jsx's
      // startWorkingLead().
