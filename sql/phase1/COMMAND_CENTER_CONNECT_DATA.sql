-- ============================================================================
-- COMMAND_CENTER_CONNECT_DATA.sql
-- One paste to connect the remaining Command Center boards to real data.
-- Run this in the Supabase SQL Editor on a database that already has A2/A5/A6/
-- A7/A8 + production_widgets installed (your current prod). It adds the two
-- pending pieces so live data flows:
--   SECTION 1 — Foundation repair: Goals + News functions (+ widget audit)
--   SECTION 2 — A9 personal per-agent self-scoped widgets (My Widgets)
--
-- Additive / forward-only / rerunnable-where-practical. No DROP TABLE, no
-- TRUNCATE, no unbounded DML. All SECURITY DEFINER functions set search_path=''.
-- Identity is always auth.uid(); personal widgets are hard-scoped to the caller's
-- own agent id and expose only the caller's own GCI. Take a DB backup first.
--
-- After running, run COMMAND_CENTER_VERIFY.sql and confirm every row = PASS.
-- Then set FRED_API_KEY in Vercel to light up Mortgage Rates.
-- ============================================================================
